...
async def test_all():
    ...
    # Send link
    mes_ids = await tg.send(USER, 'ola', files='https://s1.1zoom.ru/big0/621/359909-svetik.jpg')
    assert len(mes_ids) == 1
    ...
    mes_ids = await tg.send(
        USER,
        files={'data': 'https://github.com/postbird/Mp4ToBlob/blob/master/video/v0-new.mp4?raw=true', 'type': 'video'},
    )
    assert len(mes_ids) == 1

    # Send album with caption
    mes_ids = await tg.send(USER, 'ola', files=[f'{PATH}data/test.png', f'{PATH}data/test.png'])
    assert len(mes_ids) == 2
    ...
    mes_ids = await tg.send(
        ...
        'ola',
        files=[
            f'{PATH}data/test.png', f'{PATH}data/test.png', f'{PATH}data/test.png', f'{PATH}data/test.png',
            f'{PATH}data/test.png', f'{PATH}data/test.png', f'{PATH}data/test.png', f'{PATH}data/test.png',
            f'{PATH}data/test.png', f'{PATH}data/test.png', f'{PATH}data/test.png', f'{PATH}data/test.png',
            f'{PATH}data/test.png', f'{PATH}data/test.png', f'{PATH}data/test.png', f'{PATH}data/test.png',
            f'{PATH}data/test.png', f'{PATH}data/test.png', f'{PATH}data/test.png', f'{PATH}data/test.png',
    ...
    # Send animation
    mes_ids = await tg.send(USER, 'ola', files={'data': 'http://techslides.com/demos/sample-videos/small.mp4', 'type': 'animation'})
    assert len(mes_ids) == 1

    # Send voice
    mes_ids = await tg.send(USER, 'ola', files={'data': f'{PATH}data/test.ogg', 'type': 'voice'})
    assert len(mes_ids) == 1
    ...
    # Send document
    mes_ids = await tg.send(USER, 'ola', files={'data': f'{PATH}data/test.pdf', 'type': 'document'})
    assert len(mes_ids) == 1
